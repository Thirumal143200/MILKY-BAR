      ...
      <ScrollView className="flex-1 p-6">
        <View className="space-y-6">
          <View className="bg-gray-50 dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700">
            ...
            <Text className="text-base font-bold text-gray-900 dark:text-white mb-4">{scan?.id}</Text>

            <Text className="text-sm font-semibold text-gray-400 mb-1">TITLE</Text>
            <Text className="text-lg font-bold text-gray-900 dark:text-white mb-4">{scan?.title}</Text>

            <Text className="text-sm font-semibold text-gray-400 mb-1">NOTES</Text>
            <Text className="text-base text-gray-700 dark:text-gray-300 mb-4">{scan?.notes || 'No description provided.'}</Text>

            <Text className="text-sm font-semibold text-gray-400 mb-1">STATUS</Text>
            <View className="flex-row items-center mt-1">
              <View className={`px-3 py-1 rounded-full ${scan?.status === 'completed' ? 'bg-green-100' : 'bg-yellow-100'}`}>
                <Text className={`font-semibold text-sm ${scan?.status === 'completed' ? 'text-green-800' : 'text-yellow-800'}`}>
          ...
          {scan?.prediction && (
            <View className="bg-gray-50 dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700">
              <Text className="text-base font-bold text-gray-900 dark:text-white mb-4">AI Prediction Run</Text>

              <Text className="text-sm font-semibold text-gray-400 mb-1">QUALITY LABEL</Text>
              <Text className="text-base font-bold text-blue-600 dark:text-blue-400 mb-4 uppercase">{scan.prediction.qualityLabel}</Text>

              <Text className="text-sm font-semibold text-gray-400 mb-1">CONFIDENCE SCORE</Text>
              <Text className="text-base font-bold text-gray-900 dark:text-white mb-4">{(scan.prediction.confidence * 100).toFixed(1)}%</Text>
          ...
          <View className="space-y-3 mt-4">
            ...
            >
              <Text className="text-gray-800 dark:text-gray-200 font-bold text-base">View Report Details</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>
